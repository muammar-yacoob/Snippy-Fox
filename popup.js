  ...
  if (text) {
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
      ...
      chrome.storage.sync.get(['notes', 'closeAfterSave'], function(result) {
        ...
        const existingNoteIndex = notes.findIndex(note => note.url === currentUrl);
        ...
        chrome.storage.sync.set({ notes: notes }, function() {
          ...
          // Show Chrome notification through background script
  ...
  if (text) {
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
      ...
      chrome.storage.sync.get(['notes', 'closeAfterSave'], function(result) {
        ...
        const existingIndex = notes.findIndex(note => note.url === currentUrl);

        if (existingIndex !== -1) {
          // Update existing note
          notes[existingIndex].text += '\n\n' + text;
        ...
        chrome.storage.sync.set({ notes: notes }, function() {
          ...
          chrome.notifications.create({
            ...
            iconUrl: 'icon48.png',
            title: 'Snippy Fox',
            message: existingIndex !== -1 ? 'Note updated!' : 'Note saved!'
          });
